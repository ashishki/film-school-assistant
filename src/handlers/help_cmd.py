...
`/get <id>` — открыть запись целиком (например `/get 12`)
`/review` — список идей для разбора; `/review 12` — разобрать конкретную
...
«Перенеси вечернее напоминание на 20:00»

Команды (если нужны):
`/practices` — список с временем следующего срабатывания
`/pause_daily_practice morning` / `/resume_daily_practice evening`

🧍 Сохранить что-то о себе
«Запомни обо мне — я сейчас делаю дипломный фильм про рынок, режиссёрский факультет, 4 курс»
«Сохрани это как контекст обо мне: …»
Я буду учитывать это в разборах и советах.
